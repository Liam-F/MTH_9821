...
def Trinomial_Tree_Pricing(Op, TMT, Greek = False):
    ...
    for i in xrange(1, 2 * N + 1):
        ...
        if Op.cp == "C":
            ...
            fv[i] = max(K - fs[i], 0)
        if Op.ae == "Double":
            if fs[i] > Op.Up_Barrier or fs[i] < Op.Down_Barrier:
                fv[i] = 2
    # print fv

    Sc = fs.copy()
    for j in range(N - 1, -1, -1):
        fv[0:(2 * j + 1)] = np.exp(-r * dt) * (p_u * fv[0:(2 * j + 1)] + p_m * fv[1:(2 * j + 2)] + p_d * fv[2:(2 * j + 3)])
        if Op.ae == "AM":
            Sc *= d
            if Op.cp == "C":
                fv[0:(2 * j + 1)] = np.maximum(fv[0:(2 * j + 1)], Sc[0:(2 * j + 1)] - K)
            else:
                fv[0:(2 * j + 1)] = np.maximum(fv[0:(2 * j + 1)], K - Sc[0:(2 * j + 1)])
        if Op.ae == "Double":
            B_up, B_down = Op.Up_Barrier, Op.Down_Barrier
            Sc *= d
            up_bvec = Sc[0:(2 * j + 1)] < B_up
            down_bvec = Sc[0:(2 * j + 1)] > B_down
            fv[0 : 2 * j + 1] = 2 + (fv[0 : 2 * j + 1] - 2) * (up_bvec * down_bvec)
    ...
    if not Greek:
        ...
        return fv[0][0], float(Delta), float(Gamma), float(Theta)
#
#
