...
def finite_diff(opt, r, M=64, PDE_Solver="Backward_Euler", Linear_Solver='LU', Greek=False):
    S0, K, T, q, sigma = opt.spot, opt.strike, opt.maturity, opt.div_rate, opt.vol
    a = (r - q) / sigma ** 2 - 1 / 2
    b = ((r - q) / sigma ** 2 + 1 / 2) ** 2 + 2 * q / sigma ** 2
    x_left, x_right, tau_final = Discretization(opt, r)
...
if __name__ == "__main__":
    eup = Option(S0=41, K=40, T=0.75, q=0.02, sigma=0.35, ae='AM', cp='P')
    r = 0.04
    # print BS.Black_Scholes_Pricing(eup, r, Greek=True)
    print finite_diff(eup, r, Greek=True)
